...
2. Rensenware requires a .NET Framework 3.5 environment, which is actually a good thing for Windows 10/11 physical machine users, because at least they have the right to regret it (after all, you can choose not to install .NET Framework 3.5, thus avoiding triggering the virus). However, this is a disaster for virtual machine testing; since Windows 8.1, Microsoft no longer preinstalls .NET Framework 3.5 with Windows. If you want to install this runtime, you must not only connect your virtual machine to the internet (which is fatal for VM testing), but also wait at least 15 minutes (with bad luck, it could take over half an hour). In contrast, KanjuWare uses Python 3.8 + PyInstaller, the downside being that if you want to test it on Windows 7, you need to install all complete patch updates for Windows 7, while the upside is that it requires no additional runtimes on any system.
